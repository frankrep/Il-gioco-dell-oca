...
void main(void) {
	...
	do {
		...
		if (opzione == 1) {
			...
			if (leggere_nome_vincitore(vincitore_attuale) != FINE_STRINGA) {
				FILE_CLASSIFICA_TOP_10 = aggiornare_classifica_top_10(
						FILE_CLASSIFICA_TOP_10, vincitore_partita);
			}
		} else {
			if (opzione == 2) {
	...
	} while (uscita != RISPOSTA_AFFERMATIVA_MAIUSCOLO
			&& uscita != RISPOSTA_AFFERMATIVA_MINUSCOLO);
	system("pause");
	return;
}
...
void confermare_scelta(char *conferma_scelta) {
	...
	do {
		...
		if ((conferma_scelta != RISPOSTA_AFFERMATIVA_MAIUSCOLO)
				&& (conferma_scelta != RISPOSTA_AFFERMATIVA_MINUSCOLO)
				&& (conferma_scelta != RISPOSTA_NEGATIVA_MAIUSCOLO)
				&& (conferma_scelta != RISPSOTA_NEGATIVA_MINUSCOLO)) {
			//stampare a video il messaggio d'errore relativo alla conferma
		}
	} while ((conferma_scelta != RISPOSTA_AFFERMATIVA_MAIUSCOLO)
			&& (conferma_scelta != RISPOSTA_AFFERMATIVA_MINUSCOLO)
			&& (conferma_scelta != RISPOSTA_NEGATIVA_MAIUSCOLO)
			&& (conferma_scelta != RISPOSTA_NEGATIVA_MINUSCOLO));
	return;
}
