...
partita* calcolare_caselle_eccesso(partita* partita_attuale){
    giocatore* giocatore_attuale;
    accesso = leggere_posizione_giocatore (leggere_giocatore(partita_attuale), leggere_turno(partita_attuale)) - leggere_lunghezza_percorso(partita_attuale);
    giocatore_attuale = leggere_giocatore(partita_attuale,leggere_turno(partita_attuale));
...
partita* gestire_casella(partita* partita_attuale, char* NOME_OCA,char* NOME_PRIGIONE,char* NOME_PONTE,char* NOME_LABIRINTO,char* NOME_LOCANDA,char* NOME_POZZO,char* NOME_SCHELETRO,char* NOME_VITTORIA,int CASELLA_ARRIVO_SCHELETRO){
    ...
    do{
        ...
        else if (strcmp(nome,NOME_LABIRINTO) == 1)
        {
            ...
            while (confrontare_stringhe(leggere_nome_casella(leggere_percorso(partita_attuale, leggere_posizione_giocatore(giocatore_attuale))), FINE_STRINGA) = FALSO)
            {
                giocatore_attuale = scrivere_posizione_giocatore(giocatore_attuale, leggere_posizione_giocatore(giocatore_attuale) - 1);
            }
            partita_attuale = scrivere_giocatori(partita_attuale, giocatore_attuale, leggere_turno(partita_attuale));
        }
        else if (strcmp(nome,NOME_SCHELETRO) == 1)
        {
...
partita* gestore_oca(partita* partita_attuale,int PRIMO_LANCIO_3_6,int PRIMO_LANCIO_4_5,int DADO_MINORE_PRIMO_LANCIO,int DADO_MAGGIORE_PRIMO_LANCIO,int PRIMO_DADO,char FINE_STRINGA){
    ...
    if(leggere_lanci_effettuati(giocatore_attuale) == 1)
    {
        ...
        while(confrontare_stringhe(leggere_nome_casella(leggere_percorso(partita_attuale, leggere_posizione_giocatore(giocatore_attuale))), FINE_STRINGA) == 0)
        {
            giocatore_attuale = scrivere_posizione_giocatore(giocatore_attuale, leggere_posizione_giocatore(giocatore_attuale) - 1);
        }
        partita_attuale = scrivere_giocatori(partita_attuale, giocatore_attuale, leggere_turno(partita_attuale));
...
partita* impostare_autorizzazioni(partita* partita_attuale, char nome_casella_attuale[DIMENSIONE_MASSIMA_NOME_CASELLA]){
    giocatore* giocatore_attuale;
    int indice_giocatore=1;
    while (indice_giocatore <= leggere_numero_giocatori(partita_attuale)){
        ...
        if (confrontare_stringhe (leggere_nome_casella (leggere_percorso (partita_attuale, leggere_posizione_giocatore(giocatore_attuale))), nome_casella_attuale) == 1){
...
partita* gestire_autorizzazione(partita* partita_attuale, char NOME_LOCANDA[DIMENSIONE_MASSIMA_NOME_CASELLA], char NOME_PRIGIONE[DIMENSIONE_MASSIMA_NOME_CASELLA], int DADO_MINORE_USCITA_PRIGIONE, int DADO_MAGGIORE_USCITA_PRIGIONE){
    giocatore* giocatore_attuale = leggere_giocatore(partita_attuale, leggere_turno(partita_attuale));
    autorizzazione = leggere_atuorizzazione(giocatore_attuale);
    if (confrontare_stringhe(leggere_nome_casella(leggere_percorso(partita_attuale, leggere_posizione_giocatore(giocatore_attuale)))) == 1){
    ...
    else if(confrontare_Stringhe(leggere_nome_casella(leggere_percorso(partita_attuale,leggere_posizione_giocatore(giocatore_attuale))),NOME_PRIGIONE) ==1 ){
